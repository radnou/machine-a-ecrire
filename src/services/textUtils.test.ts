...
const createTestChar = (char: string, idSuffix: string | number = '', state: 'fresh' | 'dried' = 'dried'): CharObject => ({
  id: `test-id-${idSuffix}`,
  char,
  state,
});
  ...
  describe('charObjectsToString', () => {
    ...
    it('should convert multiple lines of characters to a newline-separated string', () => {
      const input: CharObject[][] = [
        ...
        [createTestChar('l', 3), createTestChar('l', 4), createTestChar('o', 5)],
      ];
  ...
  describe('stringToCharObjects', () => {
    ...
    it('should ensure all generated CharObjects have state "dried" and unique IDs', () => {
        ...
        const mockMathRandom = vi.spyOn(Math, 'random')
            ...
            .mockReturnValueOnce(0.234567890)  // For 'b'
