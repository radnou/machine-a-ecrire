...
describe('GitHub Sync Mock Service', () => {
  ...
  describe('loadFileFromGitHub', () => {
    ...
    it('should return a file for a known mock path', async () => {
      const response = await loadFileFromGitHub(
        MOCK_KNOWN_FILE_PATH_IN_SERVICE,
        TEST_REPO,
        TEST_TOKEN
      );
  ...
  describe('listFilesFromGitHub', () => {
    ...
    it('should return a filtered list of files for a specific directory path', async () => {
      ...
      const response = await listFilesFromGitHub(
        TEST_REPO,
        TEST_TOKEN,
        directoryPath
      );
      ...
      const knownFileInDocs = response.files?.find(
        (file) => file.path === 'docs/notes.txt'
      );
