...
export async function saveFileToGitHub(
  ...
  console.log(
    `[MockGitHubSync] Attempting to save file: ${file.path} to repo: ${repo}, token used: ${token ? 'yes' : 'no'}`
  );
  ...
  if (!file.path || typeof file.content === 'undefined') {
    return {
      ...
      path: file.path || '',
      sha: '',
      message: 'File path or content is missing.',
    };
  }
  ...
  console.log(
    `[MockGitHubSync] File saved successfully: ${file.path}, new SHA: ${fakeSha}`
  );
...
export async function loadFileFromGitHub(
  ...
  console.log(
    `[MockGitHubSync] Attempting to load file: ${path} from repo: ${repo}, token used: ${token ? 'yes' : 'no'}`
  );
...
export async function listFilesFromGitHub(
  ...
  console.log(
    `[MockGitHubSync] Attempting to list files in dir: '${directoryPath || '/'}' from repo: ${repo}, token used: ${token ? 'yes' : 'no'}`
  );
  ...
  console.log(
    `[MockGitHubSync] Files listed successfully for path: '${directoryPath || '/'}'. Found ${filteredFiles.length} items.`
  );
